...
- [ ] 单页四区块：题材&世界观、故事内核、主要角色、全本大纲
- [ ] 支持手动填写；确认后创建项目目录与初始文件
- [ ] **AI 一键生成全部**（题材/内核/人物/大纲协同生成）
- [ ] 各区块可单独 AI 生成；已填内容作为上下文补全空白项
- [ ] 部分卡片点击展开，在模态窗中详细编辑后回写
...
- [ ] 三栏布局：
  ...
  - 中间：章节编辑器（文本域）
  - 右侧：**AI Agent 对话栏**
- [ ] Agent 栏右上角：历史会话、新建对话、临时规则（遮罩层）
- [ ] 对话区展示生成结果；底栏快捷指令：续写、润色、扩写、生成；支持 `/skill`
- [ ] 章节保存功能
...
- [ ] 分类展示人物、物品、世界观、剧情要点
- [ ] 人物条目含 `name` / `profile` / `relationship: [{ type, target }]`（`type` 允许中文）
- [ ] 支持新增、编辑、删除设定条目
